...
- I changed the "local entity URIs" to use hash notation (eg <#person=Angela_Merkel>) instead of slash (eg <#person=Angela_Merkel>)
- The local entity URIs are made from the entity text, replacing punctuation with "_". But this
  may be insufficient, since the same text could mean different entities (if the NER can
  detect such situations)
- Of course it's better to use global DBpedia URIs if they can be found (as explained in
  [[NIF-example2.ttl]])
