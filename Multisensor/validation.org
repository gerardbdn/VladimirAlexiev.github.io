...
- Gerard: The EL service only issues labels without underscores, so the ones with underscores are the ouput of running older versions of the service.

*** DONE Remove Disambiguation Labels?
...
*** DONE Why nif-ann:taIdentConf is 0?
...
- Gerard: After inding no in the code and checking Babelfy's documentation, I reached the conclusion that this is normal behaviour. Take a look at this example response from Babelfy's own documentation, which contains annotations with confidence score 0: http://babelfy.org/guide#ResponseExample
...
- Gerard: missing labels are caused by using an older version of BabelNEt (2.5.1) to retrieve them. While the solution would be to use the latest version, i.e. by using the http API, I'd day it's not worth at this stage of the project. 
