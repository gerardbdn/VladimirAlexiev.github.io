#+TITLE: Mulstisensor Validation Log
#+DATE: <2016-06-28>
...
[[http://VladimirAlexiev.github.io/Multisensor/validation.html][HTML Rendered version]]
...
If you see mistakes or imprecise info (eg the scope of a particular service is described wrong, or a particular issue is in fact a non-issue),
please contact me on skype://valexiev1 and I'll correct it.
Even better, you could edit this file on github (it's a plain text file!) and send me a pull request.
...
The corresponding RDF fragments are:
#+BEGIN_SRC Turtle
<#char=1453,1461>
        a                     nif:Word ;
...
As you can see, the first enrichment (from Babelfy) is vacuous, as it doesn't have ~taClassRef~, ~taIdentRef~.
I don't understand exactly what the service does, but if it removes this enrichment, that would be good.

*** TODO Skip vacuous Babelfy enrichments
Rather than relying on the Alignment service to remove vacuous Babelfy enrichments,
it would be better if the Entity Lookup service does not emit them in the first place.

*** TODO Alignment removes nif:Phrase?
alignment.log says:
: Removed: (#char=1453,1461, rdf:type, nif:Phrase)

However, the respective node has type ~nif:Word~ not ~nif:Phrase~. Not sure what's going on
...
*** TODO One Annotation Per Pair
After consultation with Babis, we decided to change the representation as follows:
- Write annotations in their own graph <http://data.multisensorproject.eu/similarity>, outside of any SIMMO graph.
...
#+BEGIN_SRC Turtle
graph <http://data.multisensorproject.eu/similarity> {
  <http://data.multisensorproject.eu/similarity/123> a oa:Annotation;
    oa:hasBody        
  ...
  <http://data.multisensorproject.eu/similarity/124> a oa:Annotation;
    oa:hasBody        
      <http://data.multisensorproject.eu/content/53a0938bc4770c6ba0e7d7b9ca88a637f9e9c304>,
